...
What I've landed on is [sanitize](https://github.com/vgudzhev/sanitize), a proxy that sits in front of the model and checks every piece of text before it leaves my machine. On the detection side, we're not reinventing the wheel. OpenAI released Privacy Filter, a tiny local model (50M active params) trained to spot PII and secrets in a single pass, no API call needed. For the long tail of entity types it doesn't cover, GLiNER2-PII adds 42 categories across 7 languages in just 300M params. Layer those on top of regex-based secret scanners like gitleaks and Presidio, and you get a detection stack that runs entirely on your machine before anything leaves it. I'm building it with Pi Dev, and the idea is straightforward: secrets and personal data get swapped for consistent placeholders like `[[TOKEN_1]]` or `[[IP_3]]`. The model can still reason about the logs, it just never sees the real values. When the answer comes back or the model wants to run a command, the real values get put back in. If the proxy is down, nothing gets sent. It works with whatever model I'm using at the time, and the same setup can run as a shared service for a team if you need that.
